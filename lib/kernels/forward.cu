...
__global__ void project_gaussians_fwd(
    ...
    const float4 intrins, const dim3 img_size, const dim3 tile_bounds,
    const unsigned block_width, const float clip_thresh,
    float *__restrict__ covs3d, float2 *__restrict__ xys,
    float *__restrict__ depths, int *__restrict__ radii,
    float3 *__restrict__ conics, float *__restrict__ compensation,
  ...
  // p_world.z);
  float3 p_view;
  if (helpers::clip_near_plane(p_world, viewmat, p_view, clip_thresh)) {
  ...
  float3 cov2d;
  float comp;
  project_cov3d_ewa(p_world, cur_cov3d, viewmat, fx, fy, tan_fovx, tan_fovy,
                    cov2d, comp);
  // printf("cov2d %d, %.2f %.2f %.2f\n", idx, cov2d.x, cov2d.y, cov2d.z);

  float3 conic;
  float radius;
  bool ok = helpers::compute_cov2d_bounds(cov2d, conic, radius);
  if (!ok)
  ...
  helpers::get_tile_bbox(center, radius, tile_bounds, tile_min, tile_max,
                         block_width);
...
__global__ void map_gaussian_to_intersects(
    const int num_points, const float2 *__restrict__ xys,
    const float *__restrict__ depths, const int *__restrict__ radii,
    const int32_t *__restrict__ cum_tiles_hit, const dim3 tile_bounds,
    const unsigned block_width, int64_t *__restrict__ isect_ids,
  ...
  helpers::get_tile_bbox(center, radii[idx], tile_bounds, tile_min, tile_max,
                         block_width);
...
__global__ void get_tile_bin_edges(const int num_intersects,
...
__global__ void nd_rasterize_forward(
    ...
    const float3 *__restrict__ conics, const float *__restrict__ colors,
    const float *__restrict__ opacities, float *__restrict__ final_Ts,
    int *__restrict__ final_index, float *__restrict__ out_img,
...
#pragma unroll
      for (int c = 0; c < channels; ++c) {
        pix_out[c] =
            __hadd(pix_out[c], __float2half(colors[channels * g + c] * vis));
      }
      T = next_T;
...
#pragma unroll
    for (int c = 0; c < channels; ++c) {
      out_img[pix_id * channels + c] =
          __half2float(pix_out[c]) + T * background[c];
    }
  }
}

__global__ void
rasterize_fwd(const dim3 tile_bounds, const dim3 img_size,
              const int32_t *__restrict__ gaussian_ids_sorted,
              const int2 *__restrict__ tile_bins,
              const float2 *__restrict__ xys, const float3 *__restrict__ conics,
              const float3 *__restrict__ colors,
              const float *__restrict__ opacities, float *__restrict__ final_Ts,
              int *__restrict__ final_index, float3 *__restrict__ out_img,
...
__device__ void project_cov3d_ewa(const float3 &__restrict__ mean3d,
  ...
  // upper 3x3 submatrix
  mat3 W = mat3(viewmat[0], viewmat[4], viewmat[8], viewmat[1], viewmat[5],
...
__device__ void scale_rot_to_cov3d(const float3 scale, const float glob_scale,
                                   const float4 quat, float *cov3d) {
