...
inline __device__ void cov2d_to_conic_vjp(const float3 &conic,
...
inline __device__ mat3<float> quat_to_rotmat(const float4 quat) {
  ...
  return mat3(1.f - 2.f * (y * y + z * z), 2.f * (x * y + w * z),
...
inline __device__ float4 quat_to_rotmat_vjp(const float4 quat,
                                            const mat3<float> v_R) {
  float w = quat.x;
  float x = quat.y;
  float y = quat.z;
  float z = quat.w;
