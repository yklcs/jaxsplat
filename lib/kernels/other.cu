...
__global__ void kernels::tiled_memset(
    float *dst,
    size_t n_dst,
    const float *src,
    size_t n_src
) {
    for (int tid = blockIdx.x * blockDim.x + threadIdx.x; tid < n_dst;
         tid += blockDim.x * gridDim.x) {
        dst[tid] = src[tid % n_src];
    }
}

__global__ void kernels::compute_cov2d_bounds(
