...
__global__ void nd_rasterize_backward_kernel(
    const dim3 tile_bounds, const dim3 img_size, const unsigned channels,
    const int32_t *__restrict__ gaussians_ids_sorted,
    const int2 *__restrict__ tile_bins, const float2 *__restrict__ xys,
    const float3 *__restrict__ conics, const float *__restrict__ rgbs,
    const float *__restrict__ opacities, const float *__restrict__ background,
    const float *__restrict__ final_Ts, const int *__restrict__ final_index,
    const float *__restrict__ v_output,
    const float *__restrict__ v_output_alpha, float2 *__restrict__ v_xy,
  ...
  for (int idx = warp_bin_final - 1; idx >= range.x; --idx) {
    ...
    if (valid) {
      ...
      for (int c = 0; c < channels; ++c) {
        ...
        v_alpha +=
            (rgbs[channels * g + c] * T - __half2float(S[c]) * ra) * v_out[c];
...
__global__ void rasterize_bwd(
    ...
    const int2 *__restrict__ tile_bins, const float2 *__restrict__ xys,
    const float3 *__restrict__ conics, const float3 *__restrict__ rgbs,
    const float *__restrict__ opacities, const float3 &__restrict__ background,
    const float *__restrict__ final_Ts, const int *__restrict__ final_index,
    const float3 *__restrict__ v_output,
    const float *__restrict__ v_output_alpha, float2 *__restrict__ v_xy,
    float2 *__restrict__ v_xy_abs, float3 *__restrict__ v_conic,
    float3 *__restrict__ v_rgb, float *__restrict__ v_opacity) {
  auto block = cg::this_thread_block();
  int32_t tile_id =
  ...
  __shared__ int32_t id_batch[MAX_GRID_DIM];
  __shared__ float3 xy_opacity_batch[MAX_GRID_DIM];
  __shared__ float3 conic_batch[MAX_GRID_DIM];
  __shared__ float3 rgbs_batch[MAX_GRID_DIM];
...
__global__ void project_gaussians_bwd(
    ...
    const float4 intrins, const dim3 img_size, const float *__restrict__ cov3d,
    const int *__restrict__ radii, const float3 *__restrict__ conics,
    const float *__restrict__ compensation, const float2 *__restrict__ v_xy,
    const float *__restrict__ v_depth, const float3 *__restrict__ v_conic,
    const float *__restrict__ v_compensation, float3 *__restrict__ v_cov2d,
    float *__restrict__ v_cov3d, float3 *__restrict__ v_mean3d,
    float3 *__restrict__ v_scale, float4 *__restrict__ v_quat) {
  ...
  float fx = intrins.x;
  float fy = intrins.y;
  float3 p_view = helpers::transform_4x3(viewmat, p_world);
  // get v_mean3d from v_xy
  v_mean3d[idx] = helpers::transform_4x3_rot_only_transposed(
      viewmat, helpers::project_pix_vjp({fx, fy}, p_view, v_xy[idx]));
  ...
  // get v_cov2d
  helpers::cov2d_to_conic_vjp(conics[idx], v_conic[idx], v_cov2d[idx]);
  helpers::cov2d_to_compensation_vjp(compensation[idx], conics[idx],
                                     v_compensation[idx], v_cov2d[idx]);
  // get v_cov3d (and v_mean3d contribution)
  project_cov3d_ewa_vjp(p_world, &(cov3d[6 * idx]), viewmat, fx, fy,
...
__device__ void project_cov3d_ewa_vjp(const float3 &__restrict__ mean3d,
  ...
  // upper 3x3 submatrix
  mat3 W = {viewmat[0], viewmat[4], viewmat[8], viewmat[1], viewmat[5],
            viewmat[9], viewmat[2], viewmat[6], viewmat[10]};

  float3 p = {viewmat[3], viewmat[7], viewmat[11]};
  float3 t = {
      W[0][0] * mean3d.x + W[0][1] * mean3d.y + W[0][2] * mean3d.z + p.x,
      W[1][0] * mean3d.x + W[1][1] * mean3d.y + W[1][2] * mean3d.z + p.y,
      W[2][0] * mean3d.x + W[2][1] * mean3d.y + W[2][2] * mean3d.z + p.z,
  };
  float rz = 1.f / t.z;
  float rz2 = rz * rz;
  ...
  mat3 J = {fx * rz,         0.f, 0.f, 0.f, fy * rz, 0.f, -fx * t.x * rz2,
            -fy * t.y * rz2, 0.f};
  mat3 V = {cov3d[0], cov3d[1], cov3d[2], cov3d[1], cov3d[3],
            cov3d[4], cov3d[2], cov3d[4], cov3d[5]};
  // cov = T * V * Tt; G = df/dcov = v_cov
  // -> d/dV = Tt * G * T
  // -> df/dT = G * T * Vt + Gt * T * V
  mat3 v_cov = {v_cov2d.x, 0.5f * v_cov2d.y,
                0.f,       0.5f * v_cov2d.y,
                v_cov2d.z, 0.f,
                0.f,       0.f,
                0.f};
  ...
  mat3 Vt = V.transpose();
  mat3 v_V = Tt * v_cov * T;
  mat3 v_T = v_cov * T * Vt + v_cov.transpose() * T * V;
  ...
  // compute df/d_mean3d
  // T = J * W
  mat3 v_J = v_T * W.transpose();
  float rz3 = rz2 * rz;
  float3 v_t = {-fx * rz2 * v_J[2][0], -fy * rz2 * v_J[2][1],
                -fx * rz2 * v_J[0][0] + 2.f * fx * t.x * rz3 * v_J[2][0] -
                    fy * rz2 * v_J[1][1] + 2.f * fy * t.y * rz3 * v_J[2][1]};
  // printf("v_t %.2f %.2f %.2f\n", v_t[0], v_t[1], v_t[2]);
  // printf("W %.2f %.2f %.2f\n", W[0][0], W[0][1], W[0][2]);
  v_mean3d.x += v_t.x * W[0][0] + v_t.y * W[0][1] + v_t.z * W[0][2];
  v_mean3d.y += v_t.x * W[1][0] + v_t.y * W[1][1] + v_t.z * W[1][2];
  v_mean3d.z += v_t.x * W[2][0] + v_t.y * W[2][1] + v_t.z * W[2][2];
}

// given cotangent v in output space (e.g. d_L/d_cov3d) in R(6)
// compute vJp for scale and rotation
__device__ void scale_rot_to_cov3d_vjp(const float3 scale,
  ...
  mat3 v_V = {v_cov3d[0],        0.5f * v_cov3d[1], 0.5f * v_cov3d[2],
              0.5f * v_cov3d[1], v_cov3d[3],        0.5f * v_cov3d[4],
              0.5f * v_cov3d[2], 0.5f * v_cov3d[4], v_cov3d[5]};
  mat3 R = helpers::quat_to_rotmat(quat);
  ...
  mat3 v_R = v_M * S;
  v_quat = helpers::quat_to_rotmat_vjp(quat, v_R);
}
